...
static uintptr_t *pmm_bitmap = (uintptr_t*) -1;
...
void pmm_init(multiboot2_information_header_t *m_boot2_info) {
	...
	for (size_t i = 0; i < m_map_n_entries; i++) {
		...
		if(m_map[i].length >= pmm_size) {
			if (m_map[i].start_addr != (phys_addr_t) k_start) {
				if ((ptr = (void*) (phys_addr_t) m_map[i].start_addr + pmm_size - 1) < k_start) {
					break;
				}
				else {
					if ((ptr = (void*) (phys_addr_t) k_end + pmm_size - 1) <= (void*) (phys_addr_t) m_map[i].end_addr) {
						break;
					}
				}
			}
