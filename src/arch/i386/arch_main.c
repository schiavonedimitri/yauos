			...
			if (boot_info->karg_entry[i].key != NULL && strcmp(boot_info->karg_entry[i].key, "bootconsole") == 0) {
				...
				if (boot_info->karg_entry[i].value != NULL && strcmp(boot_info->karg_entry[i].value, "serial") == 0) {
					...
					if(bootconsole_init(BOOTCONSOLE_SERIAL) == -1) {
						...
						bootconsole_mem_flush_buffer(buf);
						if (bootconsole_init(BOOTCONSOLE_VGA_TEXT_MODE) == -1) {
							b_free(buf);
							return -1;
						}
...
void arch_main(uint32_t magic, multiboot2_information_header_t *m_boot2_info) {
	...
	if (bootconsole_init(BOOTCONSOLE_MEM)) {
	...
	if (smp) {
		...
		map_page(local_apic_address, local_apic_address, PROT_PRESENT | PROT_READ_WRITE | PROT_KERN | PROT_CACHE_DISABLE, false);
        map_page(io_apic_address, io_apic_address, PROT_PRESENT | PROT_READ_WRITE | PROT_KERN | PROT_CACHE_DISABLE, false);
		...
		for (size_t i = 1; i < num_cpus; i++) {
			...
			map_page(ap_stack, ap_stack_virtual, PROT_PRESENT | PROT_READ_WRITE | PROT_KERN, false);
